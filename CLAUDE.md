...
## مالکیت فایل بین دو جلسه‌ی هم‌زمان

روی این مخزن **دو جلسه‌ی AI هم‌زمان** کار می‌کنند و هر دو مستقیماً روی همین درخت
کاری در `main` کامیت می‌کنند. این عمدی است، نه اشتباه.

| حوزه | مالک |
|---|---|
| پنل ادمین (`views/pages/admin.ejs`، `admin-blog-edit.ejs`، مسیرهای `/admin*`) | جلسه‌ی A |
| وبلاگ (`blog-writer.js`، `blog-db.js`، `blog-facts.js`) | جلسه‌ی A |
| `news-db.js`، `lib/browser-lifecycle.js` | جلسه‌ی A |
| کرالرهای بازار: خودرو، کالا، شغل، مالی، طلا، ملک | جلسه‌ی A |
| ترند آینده / تایم‌لاین (`timeline-*.js`، `views/pages/future*.ejs`) | جلسه‌ی B |
| `crawler.js` — استخراج منحنی و عبارت مرتبط گوگل‌ترند | جلسه‌ی B |
| صفحات ترند (`/trends/*`) | جلسه‌ی B |
| خروجی تلگرام (`lib/tg-*.js`، `news-listener.py`) | جلسه‌ی B |
| **مشترک:** `web-test.js`، `views/layout.ejs`، `lib/ai-client.js`، `server.js` | هر دو — با احتیاط |

### قواعدی که از خرابی‌های واقعی درآمده‌اند

۱. **بلافاصله پیش از ویرایش، فایل را دوباره بخوان.** نسخه‌ی یک‌ساعته کهنه است. در
   ۱۹ آگوست یک جلسه نسخه‌ی کش‌شده‌ی `web-test.js` را کامل آپلود کرد، `blogToday`
   از locals حذف شد و کل صفحه‌ی `/admin` با «blogToday is not defined» شکست؛
   سقف `ai_daily_limit` هم از ۱٬۰۰۰٬۰۰۰ به ۱۰٬۰۰۰ برگشت. سه کامیت این رگرسیون
   را حمل کردند تا دیده شد (کامیت ترمیم: `749772a`).

۲. **هرگز کل فایل بزرگ را آپلود نکن.** ویرایش جراحی روی سرور، با اسکریپت
   دومرحله‌ای: اول همه‌ی لنگرها بررسی شوند و اگر حتی یکی دقیقاً یک‌بار پیدا نشد،
   هیچ فایلی نوشته نشود. فایل نیم‌پچ‌شده بدترین حالت است.

۳. **پیش از کامیت** `git log --oneline -5` و `git status` — و دنبال کامیتی بگرد
   که مال تو نیست.

۴. **فقط فایل‌های خودت را استیج کن** (`git commit --only <file>`). فایل‌های
   untracked جلسه‌ی دیگر — مثل `_patch1.js`، `_snippet.py`، `lib/tg-*.js` —
   کارِ در جریان‌اند، نه زباله. پاکشان نکن.

۵. **کامیتی که خرابی زنده را درست می‌کند باید پوش شود.** کامیت پوش‌نشده همان
   چیزی است که جلسه‌ی بعدی رویش می‌نویسد.

۶. وقتی قالب به متغیری ارجاع می‌دهد، مطمئن شو ورودی متناظرش در locals داخل
   `web-test.js` هنوز هست. همین شکافِ بین دو فایل بود که `/admin` را شکست.
