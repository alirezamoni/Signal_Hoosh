...
function _post(model, messages, { max_tokens = 1200, json = false } = {}) {
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      headers: {
        'Authorization': `Bearer ${orKey.get()}`,
      ...
      // ⚠️ اینجا قبلاً «let d=''; res.on('data', c => d += c)» بود.
      // c یک Buffer است و += هر تکه را جداگانه به UTF-8 تبدیل می‌کند؛ اگر
      // مرز تکه‌ی TCP وسط یک نویسه‌ی چندبایتی بیفتد (هر حرف فارسی ۲ بایت
      // است)، هر نیمه به U+FFFD تبدیل می‌شود و متن خراب می‌آید — «حمله»
      // می‌شود «حمل��». چون فقط گاهی و روی مرز تصادفی رخ می‌داد، ماه‌ها
      // بی‌سروصدا در ترجمه‌ی اخبار و متن وبلاگ نشسته بود.
      // راه‌حل: بایت‌ها دست‌نخورده جمع شوند و یک بار در پایان رمزگشایی شوند.
...
async function _run(prompt, { system, max_tokens, json, models, tag, validate } = {}) {
  if (!orKey.get()) { console.warn(`[ai-client${tag ? '/' + tag : ''}] کلید OpenRouter تنظیم نشده — پنل مدیریت ← هوش مصنوعی`); return null; }
