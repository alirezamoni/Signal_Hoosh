...
let _pausedUntil = 0;
function isPaused() { return Date.now() < _pausedUntil || !hasBudget(); }
  ...
  for (const model of (models || getModels())) {
    try {
      ...
      if (!text || result.error) {
        ...
        if (_isDailyLimit(msg)) { tripDailyBudget(msg); return null; }
        if (_isRateLimit(msg)) { _tripShort(msg); continue; }
        console.warn(`[ai-client${tag ? '/' + tag : ''}] ${model}: ${msg.slice(0, 60)}`);
        continue;
      }
...
module.exports = {
  callText, callJSON, getModels, isPaused,
