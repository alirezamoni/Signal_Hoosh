...
app.get('/news/:id', (req, res, next) => {
  ...
  page(res, 'news-detail', '/news', {
    ...
    // ⚠️ اینجا قبلاً «robots: text.length < 300 ? 'noindex, follow' : null» بود.
    // منطقش درست بود (صفحه‌ی نازک بودجه‌ی خزش می‌سوزاند) ولی مقیاسش فاجعه:
    // یک‌شبه ۵۶ هزار صفحه — ۷۱٪ کل سایت — از ایندکس بیرون رفت، بدون اینکه
    // اول اندازه گرفته شود کدام‌یک از آن‌ها واقعاً کلیک می‌گرفتند. بعدش
...
app.get('/trends/:slug', (req, res, next) => {
  ...
  // ⚠️ عنوان و توضیح قبلاً یک قالب ثابت برای همه‌ی ۲٬۴۷۴ صفحه بود:
  // «<کلیدواژه> — ترند جستجو، حجم و تاریخچه». نتیجه در سرچ کنسول:
  //
  //   «مژده نظری»                        ۱۲ ایمپرشن → ۳ کلیک  (CTR ۲۵٪)
  //   «جایگاه‌های پرسپولیس درمقابل ...»  ۶٬۱۱۵ ایمپرشن → ۲ کلیک (CTR ۰.۰۳٪)
  //
  // برای یک نام، «ترند جستجو» واقعاً جالب است. برای کسی که جدول رده‌بندی
  // می‌خواهد، وعده‌ی «حجم و تاریخچه‌ی جستجو» یعنی «آنچه می‌خواهی اینجا
  // نیست» — پس با وجود جایگاه ۶.۹ کلیک نمی‌خورد.
  //
  // این صفحه تا ۶ خبر مرتبط هم دارد. وقتی خبر دارد، عنوان همان را وعده
  // می‌دهد؛ وقتی ندارد، وعده‌ی دروغ نمی‌دهیم و همان چارچوب ترند می‌ماند.
  const nCount = news.length;
  const seoTitle = nCount
    ? `${keyword} — آخرین اخبار و روند جستجو | سیگنال هوش`
    : `${keyword} — روند جستجو در گوگل ایران | سیگنال هوش`;
  const seoDesc = nCount
    ? `${fa(nCount)} خبر مرتبط با «${keyword}» از رسانه‌های فارسی` +
      (prof.last_day ? `، تا ${faDay(prof.last_day)}` : '') +
      ` — همراه با روند جستجوی این عبارت در گوگل ایران` +
      (prof.peak_vol ? `؛ اوج ${volText} جستجو` : '') + '.'
    : `روند جستجوی «${keyword}» در گوگل ایران` +
      (prof.peak_vol ? ` — اوج ${volText} جستجو` : '') +
      (prof.days > 1 ? `، ${fa(prof.days)} روز در فهرست ترندها` : '') +
      '. نمودار تغییرات و عبارت‌های مرتبط.';
